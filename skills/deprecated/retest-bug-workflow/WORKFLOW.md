...
- **Resolve the mention by the field value's stable identity (its account/user id), NOT by the display-name string** (PM-005). A name-keyed roster lookup can return *a* wrong id that still renders as a valid mention pill — nothing errors, so a wrong recipient looks clean. Key the roster on the id; the name is only the printed label. **Fail closed:** if that id has no roster entry, or the resolved chat-id can't be tied back to the same person, **do not send** — stop and ask (interactive) or mark the notify Blocked and report (unattended). Never ping a best-guess handle. This is a **mechanical** check (`resolved_id == roster[owner_id]`) required on **every** path including unattended bot sends — a chat client renders `<@id>` as each user's own nickname, so a wrong id is invisible to the eye and only the id-equality assertion catches it.
...
5. **Recipient (mechanical, not by eye):** re-read the QA Owner field for THIS ticket and take its **account/user id**; assert the resolved `<@id>` is the roster entry for **that id** (not a name match), and the label matches the field source. If it resolves to any other handle, or can't be tied to that id → **STOP, do not send.** A rendered mention pill is not proof of the right person — same-server `<@id>`s render as each user's own nickname, so a wrong id looks just as valid. Unattended senders MUST run this assertion in code and fail closed; never rely on a human reading the dry-run.
...
| MUST resolve the recipient by the QA Owner field's **account/user id** and assert the resolved `<@id>` == the roster entry for that id — fail closed (ask, or Blocked + report) if it doesn't; NEVER send a notify whose mention resolves by a name string or to an unverified handle | Same QA Owner on two tickets rendered as two different chat ids; the later notify pinged an unrelated handle (PM-005) |
