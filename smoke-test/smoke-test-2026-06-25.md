...
### ⏳ รายการที่ยังไม่ได้ทดสอบ (Pending)

#### Creator — ยังไม่ได้ทดสอบ

| TC | Topic | สิ่งที่ต้องตรวจ |
|----|-------|----------------|
| C02 | Sidebar navigation | คลิกทุก item, ตรวจ active state |
| C03 | Topbar navigation | ทุก link, notification bell, user avatar menu |
| C06 | Media filter (functional) | คลิก tab วิดีโอสั้น/PDF/etc → ตรวจ list เปลี่ยนจริง |
| C07 | Media search | พิมพ์ keyword → ตรวจผลลัพธ์ |
| C08 | Upload/สร้างสื่อ flow | คลิก "+ สร้างสื่อ" → modal/form ที่เปิดมา |
| C09 | Media item detail | คลิก item → หน้า detail, ตรวจ edit controls |
...
#### Learner — ยังไม่ได้ทดสอบ

| TC | Topic | สิ่งที่ต้องตรวจ |
|----|-------|----------------|
| L03 | Sidebar nav (all items) | คลิกทุก item — สำรวจอาชีพ, ติวสอบ, 8 กลุ่มสาระ, ฯลฯ |
...
| L09 | Catalog filter: วิดีโอ | คลิก filter → ผลลัพธ์เปลี่ยน |
| L10 | Catalog filter: วิดีโอสั้น | คลิก filter → ผลลัพธ์ |
| L11 | Catalog filter: เอกสาร | คลิก filter → ผลลัพธ์ |
| L12 | Catalog filter: คอร์ส | คลิก filter → ผลลัพธ์ |
| L13 | Catalog filter: เส้นทางการเรียน | คลิก filter → ผลลัพธ์ |
| L14 | Catalog search | พิมพ์ "AI" / "PDF" → ผลลัพธ์ |
| L15 | Catalog sort | เปลี่ยน sort order → ผลลัพธ์ |
| L17 | Video player controls | กด play จริง → เล่นได้? seek, volume |
| L18 | Video tabs | ข้อมูลเพิ่มเติม / ซับไตเติ้ล / เอกสารสรุป / สื่ออื่นๆ |
| L19 | Comments | ดู/โพสต์ comment |
