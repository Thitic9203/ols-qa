...
Respond in English only (never Thai in user-facing text). Do not run Playwright until the user confirms the intake summary. After tests, summarize in chat, then ask if results should be updated elsewhere (see skill Phase G). Do not open bugs — use create-bug-workflow.
