# Post-Mortem Report #0056 — CI ของ repo แดงต่อเนื่อง 5 วันเพราะเทสต์สร้าง commit โดยไม่ตั้ง git identity
...
> **หมายเหตุการรวมเข้า main (2026-09-15):** รายงานฉบับนี้เขียนเสร็จสมบูรณ์เมื่อ 2026-09-11 ในเซสชันที่ทำงานบน git worktree แยก (`.claude/worktrees/serene-mendel-f2977e`) แต่คอมมิตที่แนบรายงานนี้มา (`4014021`) ไม่เคย merge เข้า `main` — worktree ถูกทิ้งค้างไว้ 4 วันโดยไม่มีใครลบหรือ push ต่อ. ค้นพบและนำโค้ด+รายงานฉบับนี้เข้า `main` เมื่อ 2026-09-15 ระหว่างสืบสาเหตุที่ `bash scripts/run-test-suites.sh` ล้มอยู่ (สาเหตุคือ worktree ที่ค้างอยู่นี่เอง ทำให้ `list-test-suites.sh` เจอไฟล์เทสต์ซ้อนอยู่ข้างในมันด้วย). เลขรายงานเปลี่ยนจาก `#0051` (เลขที่จองไว้ตอนเขียนบน worktree ที่แยกออกไป) เป็น `#0056` เพราะ `main` เดินหน้าใช้เลข 0050-0054 กับรายงานอื่นไปแล้วก่อนโค้ดนี้จะถูกนำเข้ามา (รวมถึงรายงาน #0055 ที่นำเข้าคู่กันในรอบเดียวกันนี้ — commit `36ed632` ที่เป็นพ่อของคอมมิตนี้บน worktree เดียวกัน) — เนื้อหาการสืบสวน/แก้ไข/หลักฐานด้านล่างเป็นของเดิมทั้งหมด ไม่ได้เขียนใหม่.
