...
| 0050 | 2026-09-11 | ตัวอ่านบัญชีหนี้เอง (`parseLedger()` และ `ledger_open_count.sh`) อ่านเฉพาะแถวระหว่างหัวตารางถึงบรรทัดแรกที่ไม่ใช่ `\|` — แถว `\| PM-...` ที่วางก่อนหัวตารางหรือหลังตารางจบจึงไม่เคยถูกมองเห็นเลยทั้ง Node และ shell fallback ตั้งแต่คอมมิตแรกที่เขียน (2026-09-05/06) ทำซ้ำยืนยันแล้วว่า pre-commit จริงปล่อยคอมมิตผ่านทั้งที่มีหนี้ OPEN ค้าง ผิดซ้ำจาก #0002 · #0003 (เกิดที่ตัวบัญชีหนี้เองซึ่งทั้งสองรายงานไม่เคยครอบ) | High | [20260911-post-mortem-report-0050-ledger-parser-dropped-stray-rows-outside-table.md](20260911-post-mortem-report-0050-ledger-parser-dropped-stray-rows-outside-table.md) |
| 0051 | 2026-09-11 | GitHub Actions `node-tests` แดงต่อเนื่อง 28 จาก 30 รอบมาตั้งแต่คอมมิต `8e83464` (2026-09-06) ไม่มีใครสังเกต จนพบโดยบังเอิญตอนเช็คสถานะ PR ของรายงาน #0050 — สาเหตุคือ `commitWithNoSuites()` ใน `tools/push-gate/pre_push.test.js` เรียก `git commit-tree` สร้าง commit ชั่วคราวโดยไม่เคยตั้ง git identity เครื่อง dev ทุกเครื่องมี global config อยู่แล้วจึงผ่านตลอด แต่ CI runner สดไม่มี ทำซ้ำยืนยันด้วยการจำลองสภาพแวดล้อมไม่มี git identity ในเครื่องตัวเอง ได้ error ตรงกับ CI เป๊ะ | High | [20260911-post-mortem-report-0051-ci-red-five-days-unset-git-identity-in-test-fixture.md](20260911-post-mortem-report-0051-ci-red-five-days-unset-git-identity-in-test-fixture.md) |
