...
echo -n "[7/8] Menu consistency... "
CMD_COUNT=$(ls commands/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "OK ($CMD_COUNT commands)"

# 8. Test.md exporter self-test (Initiative #1 — additive to CSV/Excel)
echo -n "[8/8] Test.md format... "
if [ -f scripts/export-test-md.py ]; then
  python3 scripts/export-test-md.py --self-test > /dev/null 2>&1 \
    && echo "OK" || { echo "FAIL: Test.md export broken"; exit 1; }
else
  echo "SKIP (export-test-md.py not found)"
fi
