...
## ติดอะไรอยู่ — เขียนใหม่ท้าย session 2026-08-25 บ่าย (เจ้าของงานสั่ง: ไม่ต้องเปิดบั๊ก โน้ตไว้พอ)

ตัวเลขทุกตัววัดสดใน session นี้ · **C1 = 94/151 = 62.3%** · เคสในชุด 277 · กลุ่ม D เหลือ **42 จาก 54**

**ปิดไปแล้วใน session นี้ — อย่าสืบซ้ำ**

- ✅ **G2 พิสูจน์แล้ว** — `browser_evaluate` + `browser_run_code_unsafe` ถูกปฏิเสธจริง และ **control** `browser_navigate`
  ยังผ่านใน session เดียวกัน (ถ้าไม่มี control การปฏิเสธแยกไม่ออกจากเซิร์ฟเวอร์ตาย) · หลักฐาน `.claude/README-mcp-guard.md`
  ในรีโปโค้ด · รูปแบบจริงคือ **ไม่เปิดชื่อ tool ให้เห็นเลย** ไม่ใช่ดักตอนเรียก
- ✅ **Feed 8 ใบ + Nav/Sidebar 5 ใบ ปิดครบ** (`29d25d5` `8814c8f` `1848fbd` `8fefebb` `9acb09f`)
- ✅ **สืบ skip 45 ใบครบ** — PRESENT 4 · SEEDABLE-STATE 23 · NEEDS-CONTENT 11 · ถามผิดคำถาม 7 ⇒ **QA ทำเองได้ 27 ใบ**
- ✅ **`AXX-002` เขียว** · ✅ **row 1b ปิด** — สมมติฐาน "reject ก่อนแล้วลบ" **ผิด** สถานะคิวไม่ใช่ตัวบล็อกการลบ

**ยังติดอยู่**

1. 🔴 **`FEED-123` · `FEED-124` เป็น flaky** ในรอบบ่าย — เป็นเคสที่เพิ่งเขียนวันนี้ **ยังนับว่าปิดจริงไม่ได้จนกว่าจะนิ่ง**
2. **เคสตกใหม่ 4 ใบยังไม่สืบ** — `ASX-005` `CRS-007` `PRF-006` `MLV-034` (อีก 6 ใบเป็นชุดเดิม)
3. **สื่อค้างคิว "รออนุมัติ" 45 ชิ้นยังอยู่** — ลบไม่ได้จริง (`409 media.not_deletable` แม้สถานะ `REJECTED`) = เจ้าของงานเคาะ
4. **OLS-609…619 ทั้ง 11 ใบ `assignee` ว่าง · 0 comment ตั้งแต่ 2026-08-20** ⇒ ที่เงียบมา 5 วันคือ **ยังไม่เคยมีใครถูกถาม**
   ไม่ใช่ PO ไม่ตอบ · จะมอบหมายให้ใครเป็นการตัดสินใจของเจ้าของงาน
5. **รอเจ้าของงานเคาะเหมือนเดิม** — `CNT-004` · `OLS-325` ครอบ 320px ไหม · แก้ `ssoEmbed` แยก env ไหม

**กับดัก 2 อย่างที่เพิ่งเสียเวลาไป — อย่าเจอซ้ำ**

- **MCP scope local ผูกกับโฟลเดอร์ที่ "เปิด session"** ไม่ใช่โฟลเดอร์ที่ทำงาน · session ที่เปิดจาก `ols-qa` จะไม่มี
  `playwright-ms` เลย แม้ `claude mcp list` จาก `ols-qa-e2e` จะขึ้น Connected และแม้สั่งย้าย directory ระหว่างทาง
  ⇒ **งานที่ต้องใช้ planner/`browser_snapshot` ต้องเปิด session ที่ `ols-qa-e2e` ตั้งแต่คำสั่งแรก**
- **`cmd | tail` คืน exit code ของ `tail` ไม่ใช่ของ `cmd`** — รอบนี้เกือบ commit ของที่ยังไม่พิสูจน์เพราะอ่าน `rc=0` ผิดตัว ·
  และ `rtk proxy npx playwright test <file>` **กลืน path ทิ้ง** กลายเป็นรันทั้งชุด 310 เคสบน pre-prod

**Jira:** `OLS-608` `[QA Task] OLS E2E Automate` · `OLS-620` (สืบ skip 45) **เสร็จแล้ว** · `OLS-621` (รหัสเคสมีวงเล็บ) โค้ดแก้แล้ว
`dcc5eaa` · `OLS-622` (พิสูจน์ deny-list) **เสร็จแล้ว** — ทั้ง 3 ใบยังขึ้น `To Do` บนบอร์ด รอเจ้าของงานปิด
