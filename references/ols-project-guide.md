...
- **Resolve by `accountId`, verify, fail closed (2026-07-25, PM-005).** `customfield_12120` returns the QA Owner's `accountId` (an opaque Atlassian account id) — map **that id** to the Discord id via the roster (memory `reference_ols-discord-qa-notify`), **not** the display-name string. Before sending, assert the resolved Discord id is the roster entry for that `accountId`; if it resolves to a different handle, or the `accountId` is not in the roster → **do not send**, ask the user (interactive) or mark Blocked and report (unattended). Two OLS retest notifies with the *same* QA Owner once pinged two different Discord ids because resolution keyed on the name string (OLS-203 correct, OLS-248 wrong). The roster must therefore be **keyed on `accountId`**; the display name is only the printed label. Keep the roster (accountId → Discord id) and webhook in local memory / `~/.ols-qa-secrets` — never in this public repo.
