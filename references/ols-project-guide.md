...
> - **หลักฐานการทดสอบ regression ลงโฟลเดอร์ Drive `<EVIDENCE_DRIVE_FOLDER_4>` เท่านั้น** (regression evidence) — **ไม่ใช่** `<EVIDENCE_DRIVE_ROOT_ID>` (`Capture screen (OLS)`) ที่เป็นของ per-ticket. โครง = **แยกชั้น verdict (`passed`/`failed`) ก่อน แล้วแยก subfolder ต่อ Test Case ID**: `<EVIDENCE_DRIVE_FOLDER_4>/<passed|failed>/<TC-ID>/<files>` (`drive_upload.py --parent <EVIDENCE_DRIVE_FOLDER_4> --verdict <passed|failed> --tc <TC-ID> …`). ใช้ชื่อ verdict ตัวพิมพ์เล็ก `passed` / `failed` เสมอ (กันโฟลเดอร์ซ้ำ).
...
> - 🧊 **Frozen tested rows (Lot2 — บังคับ):** เคสใน `OLS: TC List (Lot2)` ที่ **มี verdict ใน col I " SKL-QA Test Status" แล้ว** (PASSED — และรวม FAILED/BLOCKED/PWMI) **ห้าม re-sync/เขียนรายละเอียดทับเด็ดขาด**. เหตุ (user 2026-08-05): verdict นั้นถูกบันทึกไว้กับ detail เวอร์ชันตอนเทส — ถ้า detail ต้นทางเปลี่ยนแล้วซิงก์ทับ จะขัดกับผลที่เทสผ่านไปแล้ว. บังคับแล้ว: `lot2_regression_auto.py` เป็น append-only (เขียนแถวใหม่ใต้ last row เท่านั้น + gate เช็ค landing ว่าง + frozen-key assert) → แถว verdict ไม่มีทางโดนแตะ; `lot2_regression_sync.py` (curated seeder, retired) มี guard ปฏิเสธทั้ง write ถ้าเจอแถวใดมี verdict ใน col I. **col I ที่ยังไม่เทส = `READY TO TEST`/`NOT STARTED`/ว่าง เท่านั้นที่นับว่า sync ได้.**

| Field | Value |
|-------|-------|
| Sheet | https://docs.google.com/spreadsheets/d/<CUSTOMER_UAT_SHEET_ID>/edit |
| **Regression evidence Drive** | `<EVIDENCE_DRIVE_FOLDER_4>` → https://drive.google.com/drive/folders/<EVIDENCE_DRIVE_FOLDER_4> — regression evidence ลงที่นี่เท่านั้น · โครง `<passed\|failed>/<TC-ID>/<files>` (แยกชั้น verdict ก่อน แล้วต่อ TC-ID) |
