...
**OLS Workspace version: v1.15.7** (21 Jul 2026) — based on helix v1.5.31
...
own per-ticket TC tab already has **more than one (≥2)** test case in any status other than `Not Started`
...
- Tool: `~/ols-qa-testing-bot/scan_testing_flip.py` — reads the sheet **Summary** tab → each
  READY-TO-TEST story's own tab, counts `started` (status non-empty and not `Not Started`); `started ≥ 2`
  = flip candidate.
...
| **Auto-flip to TESTING** (`ols-flip-testing`) | Mon–Fri 10:30 AM | Flips any story at **READY TO TEST** whose sheet TC tab has **≥2 started cases** to **TESTING**, QA Owner untouched (see *Auto-flip stories to TESTING*) |
...
- **งานใหม่ (launchd bot, Mon–Fri 10:30)** — สแกน story ที่ Jira status = **READY TO TEST** แล้วเช็ค TC ในชีท; ถ้าแท็บของ story มีเคสสถานะ **ที่ไม่ใช่ `Not Started` ตั้งแต่ 2 เคสขึ้นไป** (= เริ่มเทสแล้ว) → flip Jira story เป็น **TESTING** อัตโนมัติ, **QA Owner คงเดิม** (ไม่เคยส่ง `assignee`)
