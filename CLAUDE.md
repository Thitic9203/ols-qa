...
## Jira attachment + Chrome MCP tool selection

The browser-JS file-attachment recipe (upload via `mcp__Control_Chrome__execute_javascript`) and the
Chrome MCP tool-selection table now live in
[references/jira-chrome-workarounds.md](references/jira-chrome-workarounds.md) — read that file before
uploading a Jira attachment via browser JS or choosing between the Chrome MCP tools in an interactive
session. (Unattended `/bot-testing` bots stay headless and write Jira over the REST API — never a browser.)
