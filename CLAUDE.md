...
**ไม่แน่ใจว่านับไหม = นับ ให้จดไว้ก่อน** ต้นทุนของแถวที่จดเกินคือหนึ่งบรรทัด ต้นทุนของความผิดพลาดที่ไม่ถูกจด
...
- **การตัดสินอยู่ที่ `investigation_rules.js` ที่เดียว** ชั้น bash พิมพ์ตัวกฎอย่างเดียว ไม่ถือ regex ของตัวเอง — สอง runtime
  ที่ถือกฎชุดเดียวกันคือสองคำตอบที่รอวันไม่ตรงกัน
...
- **ก่อน push ชุดเทสต์ต้องเขียวบน commit ที่กำลังจะขึ้น** ไม่ใช่บน working tree — บังคับด้วย `scripts/hooks/pre-push`
  ซึ่งกาง worktree ของ sha นั้นออกมารันจริง (ไฟล์ค้างของ session อื่นทำให้ผลบน working tree ตอบผิดได้ทั้งสองทาง)
